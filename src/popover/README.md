...
通过`isControl` `v2.10.0` 属性开启受控模式。在受控模式下，`show`状态完全由外部控制，需要手动控制`show`的值。
...
export default function Demo() {
  ...
  return (
    <Popover 
      ...
      placement="top"
      slot={{
        overlay: <View>受控模式下，点击不会触发 show-change 事件</View>
      }}
    >
      <Button onClick={() => setControlledShow(!controlledShow)}>受控模式</Button>
    </Popover>
  )
}
```
