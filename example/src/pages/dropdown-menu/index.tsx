...
export default function Demo() {
  ...
  const onBeforeChange = useCallback(({ detail: { status, callback } }) => {
    showModal({
      ...
      content: `${Strings.getLang('areYouSure')}${
        status ? Strings.getLang('open') : Strings.getLang('close')
      }${Strings.getLang('dropdownMenu')}`,
      success: res => {
        if (res.confirm) {
          callback(true);
        } else if (res.cancel) {
      ...
      <DemoBlock title={Strings.getLang('customMenuContent')}>
        <DropdownMenu>
          ...
          <DropdownItem id="item" title={Strings.getLang('filter')}>
            <Cell
              title={Strings.getLang('freeShipping')}
              titleStyle={{ color: switch1 ? '#3678E3' : '' }}
            >
              <Switch
                size="24px"
                style={{ height: '26px' }}
            ...
            <Cell
              title={Strings.getLang('groupBuying')}
              titleStyle={{ color: switch2 ? '#3678E3' : '' }}
            >
              <Switch
                size="24px"
                style={{ height: '26px' }}
            ...
            <View style={{ padding: '5px 16px' }}>
              <Button type="info" block round onClick={onConfirm}>
